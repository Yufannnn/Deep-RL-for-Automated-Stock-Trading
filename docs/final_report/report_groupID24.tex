...
Against this backdrop, DQN and DDQN land in the middle of the baseline ordering rather than at the bottom. DQN finishes with a clearly positive cumulative return of $+49.6\%$ and a Sharpe of about $0.84$. This places it above the Random Forest classifier on every metric and above DDQN on every metric, yet below both ARIMA and momentum. Its maximum drawdown of $-32.4\%$ is slightly worse than that of buy-and-hold at $-31.4\%$, so we cannot claim that the drawdown-shaped reward actually reduced worst-case drawdown on this split. DDQN ends with a small negative cumulative return and a near-zero Sharpe, consistent with its more conservative target formulation producing systematically smaller $Q$-values and, in this regime, translating into missed long positions during the post-crash recovery rather than any visible gain in stability.

Taken together, the experiment illustrates three lessons from first principles. First, the gap between an RL agent and buy-and-hold is not automatic evidence of a hard problem, because simple classical rules like 20-day momentum can also beat buy-and-hold on this particular trending asset. Second, a textbook from-scratch DQN is not a magic replacement for well-understood trading signals, and in this setting it underperforms both ARIMA and momentum while outperforming a Random Forest trained on the same features. Third, the difference between DQN and DDQN is visible not as a simple better-or-worse gap but as a qualitatively different policy. The DQN episode walkthrough in Appendix~\ref{app:results} (Fig.~\ref{fig:walkthrough}) shows that DQN commits to long positions for extended stretches and only retreats to flat around the sharpest drawdowns, whereas the DDQN portfolio trajectory in Fig.~\ref{fig:training} stays far closer to the zero-position line, matching its more conservative target and producing fewer decisive long or short commitments. Both policies are internally plausible given their respective target formulations, and the goal of the project, namely understanding how textbook DQN variants actually behave on real market data relative to simple classical rules, is met.
